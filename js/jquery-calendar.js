...
jQuery.extend(PopUpCal.prototype, {
	...
	setDefaults: function(settings) {
		jQuery.extend(this._defaults, settings || {});
	},
	...
	_inlineCalendar: function(target, inst) {
		jQuery(target).append(inst._calendarDiv);
	...
	dialogCalendar: function(dateText, onSelect, settings, pos) {
		...
		jQuery.extend(inst._settings, settings || {});
...
this).css({opacity:'1.0',cursor:''});
	...
	showFor: function(target) {
		...
		jQuery.extend(inst._settings, (fieldSettings ? fieldSettings(input) : {}));
...
function PopUpCalInstance(settings, inline) {
	...
	this._settings = jQuery.extend({}, settings || {}); // clone
}
